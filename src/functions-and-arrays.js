...
function sum(anyArray) {
  if (!anyArray.length) return 0;

  let sum = 0;

  for (let el of anyArray) {
    if (typeof el === "object") {
      throw new Error("Unsupported data type sir or ma'am");
      return;
    }
    //strings should have their length added to the total
...
function averageWordLength(words) {
  ...
  for (let word of words) {
    // I take the length of the array than put in the var wordLength
...
function avg(arr) {
  if (!arr.length) return null;
  let avgValue = (sum(arr) / arr.length).toFixed(2);
